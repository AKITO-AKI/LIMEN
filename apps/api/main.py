...
from meaning import estimate_meaning
from storage import ensure_db, get_session, list_sessions, save_session

app = FastAPI(title="LIMEN API", version="0.1.0")
...
def meaning_estimate(payload: dict):
    if not isinstance(payload, dict) or "inputSkeleton" not in payload:
        raise HTTPException(status_code=400, detail="Missing inputSkeleton")
    return estimate_meaning(payload)
